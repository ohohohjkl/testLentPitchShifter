...
namespace stk {
	...
	*/
	...
	public:
		//! Class constructor.
		LentPitShift(StkFloat periodRatio = 0.75, int tMax = RT_BUFFER_SIZE);
		...
		void setPeriod(StkFloat period);

		//! Input one sample to the filter and return one output.
		StkFloat tick(StkFloat input);


		StkFloat tick(StkFloat input, FILE *fout);
	...
	inline StkFloat LentPitShift::tick(StkFloat input, FILE *fout)
	{
		StkFloat sample;
